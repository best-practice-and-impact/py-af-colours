...
uk_population = pd.read_csv("data/uk_population_by_country.csv")
population_data = uk_population.drop("Country", axis = 1).reset_index()

colours = af_colours("categorical", "hex", 6)
categorical = sns.set_palette(sns.color_palette(colours))

percentage_labels = []
for n in np.arange(population_data.shape[0]):
    percentage = 100 * ((population_data.iloc[n].values.item())
                    /(population_data.values.sum().item()))
    value = round(percentage, 1)
    country = str(data["Country"].iloc[n])
    percentage_labels.append((country + " " + str(value) + "%"))

ax = most_recent.plot.pie(x = "Country", y = "Population Estimate",
                          ...
                          wedgeprops = {"edgecolor": "white",
                                        ...
                                        "antialiased": True})
ax.set_ylabel(None)
ax.set_title("Percentage Population of UK Countries in 2022")
