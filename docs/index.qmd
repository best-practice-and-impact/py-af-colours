...
This page contains the documentation for the Python package containing the
UK Government Analysis Function’s colour palettes.
The [py_af_colours package](https://github.com/best-practice-and-impact/py-af-colours)
aims to maintain the accessibility of graph outputs generated in Python across the Analysis 
Function by simplifying the process by which users can access the 
standardised colour palettes.

py_af_colours sits alongside an R equivalent, [af_colours](https://github.com/best-practice-and-impact/afcolours).
...
It is not recommended for the user to specify a value for this parameter (see Basic examples)
This should not be changed unless the user moves the file.
...
See also that a warning is given for colours lists greater than four to let the user know that for accessibility
best practice, it is prefered to use a limit of four categories.
...
## Maintenance
The Analysis Function colour palettes are stored inside the config file as hex codes,
so that in the event of future updates to the colours, package maintenance can be
